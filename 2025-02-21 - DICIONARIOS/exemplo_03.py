...
dictCursosCampus = dict(filter(
    lambda dicionario: dicionario[1]['diretoria'].split('/')[-1].strip() == strCampus,
    dictDadosCursos.items()))

# Exibe os dados dos cursos
for k, v in dictCursosCampus.items():   
   print(f'\n{k}: {v}')

print(f'\n{len(dictCursosCampus)} Cursos listados...\n')